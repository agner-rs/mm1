...
use mm1::core::context::{Messaging, Ping};
...
fn test_cross_node_ping() {
    ...
    std::thread::scope(|s| {
        ...
        rt.block_on(async move {
            ...
            let _: mm1::multinode::proto::RegisterProtocolResponse = ctx_1
                .ask(
                    ...
                    mm1::multinode::proto::RegisterProtocolRequest {
                        name:     "proto".into(),
                        protocol: Protocol::new().with_type::<mm1::proto::system::Ping>(),
                    },
            ...
            let ctx_1_addr = ctx_1.address();
            let ctx_2_addr = ctx_2.address();

            let duration = ctx_1
                .ping(ctx_2_addr, Duration::from_secs(2))
                .await
                .expect("ctx_1.ping(ctx_2)");

            info!("Cross-node ping from node1 to node2 took: {:?}", duration);
            assert!(duration < Duration::from_secs(2));

            let duration = ctx_2
                .ping(ctx_1_addr, Duration::from_secs(2))
                .await
                .expect("ctx_2.ping(ctx_1)");

            info!("Cross-node ping from node2 to node1 took: {:?}", duration);
            assert!(duration < Duration::from_secs(2));
...
#[test]
fn test_cross_node_ping_multiple() {
    mm1_logger::init(&logger_config()).ok();
    let temp_dir = TempDir::new().expect("temp-dir");
    let socket_url = format!(
        "uds://{}",
    ...
    std::thread::scope(|s| {
        ...
        rt.block_on(async move {
            ...
            for i in 1..=5 {
                ...
                info!(
                    "Cross-node ping {} from node1 to node2 took: {:?}",
                    i, duration
                );
                assert!(duration < Duration::from_secs(2));
            }
            ...
            for i in 1..=5 {
                ...
                info!(
                    "Cross-node ping {} from node2 to node1 took: {:?}",
                    i, duration
                );
                assert!(duration < Duration::from_secs(2));
            }
